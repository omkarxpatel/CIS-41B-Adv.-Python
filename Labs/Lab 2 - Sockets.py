...
class UserInterface:
    def __init__(self):
        ...
        self.plot_button = tk.Button(self.root, text="Plot", command=self.plot_data) # button to start plotting
        self.plot_button.pack()
        
    def plot_data(self):
        country = self.country_entry.get()
        BusinessLayer().process_request(country) # calls the buisinesslayer with a country
...
class BusinessLayer:
    def process_request(self, country):
        query = f"SELECT * FROM data WHERE country = '{country}' AND year BETWEEN 1970 AND 2020" # sql query to get the data from datalayer
...
class DataLayer:
    def execute_query(self, query):
        ...
        return data # returs the data found (year and co2_level)
...
class Server:
    ...
    def start(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
            ...
            # starts the server and listens on the port for data and to send data
...
class Client:
    ...
    def request_data(self, query):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            ...
            client_socket.sendall(query.encode()) # connects to the socket then sends the query
