...
def create_database(filename):
    ...
    cursor.execute("CREATE TABLE emissions (State TEXT, " +
                   ", ".join([f"_{year} REAL" for year in range(1970, 2020)]) +
                   ")")
    ...
    for i, line in enumerate(lines[3:]):
        ...
        emissions = [float(value.strip()) if value.strip() != '' and value.strip() not in ['(1970-2020)', '(2019-2020)'] else None for value in values[1:]]

        placeholders = ", ".join(["?" for _ in emissions])
        query = f"INSERT INTO emissions VALUES (?, {placeholders})"
        values = (state,) + tuple(emissions)
...
if __name__ == "__main__":
    ...
    # Initialize layers
    data_layer = DataLayer("emissions.db")
    business_layer = BusinessLayer(data_layer)
    user_interface = UserInterface(business_layer)
    server_layer = ServerLayer(data_layer)
