...
#   GENERATE PLAYLIST   #
#########################
auth = spotipy.Spotify(auth=spotipy.SpotifyOAuth(clientID, clientSecret, redirectURI)

def get_info():
    spotifyObject = auth.get_cached_token()["access_token"])    
    playlist_url = playlist_link.get()
    
    return [spotifyObject, playlist_url]    
