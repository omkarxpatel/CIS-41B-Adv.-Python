...
table = BeautifulSoup(requests.get('https://en.wikipedia.org/wiki/List_of_countries_by_carbon_dioxide_emissions_per_capita').content, 'html.parser').find('table', {'class': 'wikitable sortable'}).find_all('tr')[1:]
d = defaultdict(dict)


rs = table.find_all('tr')[1:]
for r in rs:
    cols = r.find_all('td')
