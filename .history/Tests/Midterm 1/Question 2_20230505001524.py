...
for row in rows[1:]:
    ...
    cols = [re.findall(r'\b(?:Carbon dioxide|Methane|Nitrous oxide|Tropospheric ozone)\b', col)[0] if any(word in col for word in ['Carbon dioxide', 'Methane', 'Nitrous oxide', 'Tropospheric ozone']) else re.findall(r'\d+(?:\.\d+)?', col) for col in cols]  # extract gas names and values
...
def main():
    ...
    for row in rows[1:]:
        ...
        # for col in cols:
            # columns.append(col.text.strip().strip("/").strip("xa0ppb"))
        cols = [col.text.strip() for col in cols]
        gas_list.append(cols)
