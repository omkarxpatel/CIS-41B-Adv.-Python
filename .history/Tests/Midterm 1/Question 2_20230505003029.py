...
def main():
    ...
    for row in rows[1:]:
        cols = row.find_all('td')
        cols = [col.text.strip().replace("ppm", "").replace("ppb", "").replace("%", "")replace("%", "") for col in cols]
        cols = [col.split('[')[0].split('\xa0')[0] if '[' in col else col.split('\xa0')[0] for col in cols]
    ...
    data = [
        GreenhouseGasData("Carbon dioxide", int(gas_list[0][1]), int(gas_list[0][2]), int(gas_list[0][3]), int(gas_list[0][4])),
        GreenhouseGasData("Methane", int(gas_list[1][1]), int(gas_list[1][2]), int(gas_list[1][3]), int(gas_list[1][4])),
        GreenhouseGasData("Nitrous oxide", int(gas_list[2][1]), int(gas_list[2][2]), int(gas_list[2][3]), int(gas_list[2][4])),
        GreenhouseGasData("Fluorinated gases", int(gas_list[3][1]), int(gas_list[3][2]), int(gas_list[3][3]), int(gas_list[3][4])),
    ]
