...
def main():
    ...
    for row in rows[1:]:
        cols = row.find_all('td')
        cols = [col.text.strip().replace("ppm", "").replace("ppb", "").replace("%", "") for col in cols]
        cols = [col.split('[')[0].split('\xa0')[0] if '[' in col else col.split('\xa0')[0] for col in cols]
    ...
    data = [
        ...
        GreenhouseGasData("Nitrous oxide", int(gas_list[2][1]), int(gas_list[0][2]), int(gas_list[0][3]), int(gas_list[0][4])),
        GreenhouseGasData("Fluorinated gases", int(gas_list[0][1]), int(gas_list[0][2]), int(gas_list[0][3]), int(gas_list[0][4])),
    ]
