...
class WebScraping:
   ...
   def extract_data(self):
       ...
       for row in rows:
           ...
           data['Absolute_Increase Since 1750'].append(float(cols[3]))
           data['Percentage_Increase Since 1750'].append((cols[4].rstrip('%')))
...
def main():
   ...
   for i in range(len(data['Gas'])):
       gas_data = GreenhouseGasData(data['Gas'][i], data['Pre-1750'][i], data['Recent'][i], data['Absolute Increase Since 1750'][i], data['Percentage Increase Since 1750'][i])
