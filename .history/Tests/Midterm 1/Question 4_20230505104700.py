...
class Database:
    ...
    def insert(self, query, df):
        try:
            ...
            print(list(df.itertuples())); exit()
            for row in df.itertuples():
                cursor.execute(query.format(row[0], row[1]), row[2], row[3], row[4])
...
colandType = {'gas': 'STRING', 'Pre_1750': "INTEGER", "Recent":"INTEGER", "Absolute_increase_since_1750":"INTEGER", "Percentage_increase_since_1750":"STRING"}

db.table(db.query_builder("Gasses", "TABLE", colandType))
db.insert(db.query_builder("Gasses", "INSERT", colandType), dataframe)
cursor = sqliteConnection.cursor()
