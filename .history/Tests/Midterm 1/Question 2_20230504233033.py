...
class GasConcentrationData:
    GasTuple = namedtuple('GasTuple', ['Gas', 'Pre_1750', 'Recent', 'Absolute_increase_since 1750', 'Percentage increase since 1750'])
    gas_data = []
    ...
    def __repr__(self):
        return f"GasConcentrationData({self.gas_data})"

    def __eq__(self, other):
        if isinstance(other, GasConcentrationData):
            return self.gas_data == other.gas_data
        return False
    ...
    def sort_by_column(self, column_name):
        self.gas_data = sorted(self.gas_data, key=lambda x: getattr(x, column_name))
...
GasConcentration = GasConcentrationData(gas_list)

# example usage
print(GasConcentration.search_gas('Carbon dioxide'))
GasConcentration.sort_by_column('Recent')
