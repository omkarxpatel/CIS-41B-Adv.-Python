...
for row in rows[1:]:
    ...
    gas_name = cols[0].text.strip().split(' ')[0]  # get the gas name and remove (CO2), (CH4), etc.
    values = [col.text.strip().split(' ')[0] for col in cols[1:]]  # get the values and remove 'ppm' and 'ppb'
    gas_list.append([gas_name] + values)
